...
class MicrocksContainersEnsembleTest {
   ...
   @Test
   void testMockingFunctionality() {
      try (
            MicrocksContainersEnsemble ensemble = new MicrocksContainersEnsemble(TestConstants.LATEST_IMAGE)
   ...
   void testPostmanContractTestingFunctionality() throws Exception {
      try (
            MicrocksContainersEnsemble ensemble = new MicrocksContainersEnsemble(TestConstants.LATEST_IMAGE).withPostman();

            GenericContainer<?> badImpl = new GenericContainer<>(TestConstants.BAD_PASTRY_IMAGE)
            ...
            GenericContainer<?> goodImpl = new GenericContainer<>(TestConstants.GOOD_PASTRY_IMAGE)
                  .withNetwork(ensemble.getNetwork())
   ...
   void testAsyncFeatureSetup() {
      MicrocksContainersEnsemble ensemble = new MicrocksContainersEnsemble(TestConstants.LATEST_NATIVE_IMAGE).withAsyncFeature();

      assertEquals(TestConstants.ASYNC_LATEST_IMAGE, ensemble.getAsyncMinionContainer().getDockerImageName());
   }

   @Test
   void testAsyncFeatureMockingFunctionality() {
      try (
            MicrocksContainersEnsemble ensemble = new MicrocksContainersEnsemble(TestConstants.LATEST_IMAGE)
                  .withDebugLogLevel()
   ...
   void testAsyncFeatureTestingFunctionality() throws Exception {
      try (
            ...
            GenericContainer<?> badImpl = new GenericContainer<>(TestConstants.BAD_PASTRY_ASYNC_IMAGE)
                  .withNetwork(ensemble.getNetwork())
                  .withNetworkAliases("bad-impl")
                  .waitingFor(Wait.forLogMessage(".*Starting WebSocket server on ws://localhost:4001/websocket.*", 1));
            GenericContainer<?> goodImpl = new GenericContainer<>(TestConstants.GOOD_PASTRY_ASYNC_IMAGE)
                  .withNetwork(ensemble.getNetwork())
                  .withNetworkAliases("good-impl")
